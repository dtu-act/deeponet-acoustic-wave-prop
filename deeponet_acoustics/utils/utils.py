...
def calcErrors(p_pred, p_ref, x0, r0, f):
    ...
    threshold_dB = 60
    indxs_ref = np.where(
        20 * np.log(np.abs(p_ref / np.max(abs(p_ref)))) > -threshold_dB
    )[0]
    indxs_pred = np.where(
        20 * np.log(np.abs(p_pred / np.max(abs(p_ref)))) > -threshold_dB
    )[0]
    ...
    f.write(
        f"Relative err: {np.round(mean_err_rel * 100, 1)}% / {np.round(20 * np.log10(1 - mean_err_rel), 1)} dB\n"
    )
