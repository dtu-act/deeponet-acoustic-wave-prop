...
from deeponet_acoustics.datahandlers.datagenerators import (
    DataGenerator,
    normalizeDomain,
    normalizeFourierDataExpansionZero,
)
...
def train(settings_dict):
    ...
    u_train, s_train, t1d, grid1d = setupData(
        data.mesh, data.pressures, data.upressures, data.t, data.ushape, flatten_data
    )
    u_val, s_val, t1d_val, grid1d_val = setupData(
        data_val.mesh,
        data_val.pressures,
        data_val.upressures,
        data_val.t,
    ...
    if settings.normalize_data and from_zero:
        ...
        y_train = normalizeFourierDataExpansionZero(
            y_train, data_nonfeat_dim=data_nonfeat_dim
        )
        y_val = normalizeFourierDataExpansionZero(
            y_val, data_nonfeat_dim=data_nonfeat_dim
        )
    ...
    if settings.transfer_learning is None:
        dataset = DataGenerator(
            u_train,
            y_train,
            s_train,
            training.batch_size_branch,
            training.batch_size_coord,
        )
        dataset_val = DataGenerator(
            u_val, y_val, s_val, training.batch_size_branch, training.batch_size_coord
        )
    else:
        flatten_data = branch_nn.network_type != NetworkArchitectureType.RESNET
        u_src_train, u_src_val = setupTransferLearningData(
            settings.transfer_learning, flatten_data=flatten_data
        )
        dataset = DataGenerator(
            u_train,
            y_train,
