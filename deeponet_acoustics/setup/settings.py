...
class SimulationSettings:
    ...
    def __init__(self, settings, input_dir=None, output_dir=None):
        ...
        self.branch_net = setupNetworkArchitecture(
            settings["branch_net"], num_output_neurons
        )
        self.trunk_net = setupNetworkArchitecture(
            settings["trunk_net"], num_output_neurons
        )

        if "transfer_learning" in settings:
            self.transfer_learning = TransferLearning(settings, self.dirs.models_dir)

            num_frozen_layers_bn = (
                len(self.transfer_learning.freeze_layers["bn"])
                if "bn" in self.transfer_learning.freeze_layers
                else 0
            )
            num_frozen_layers_tn = (
                len(self.transfer_learning.freeze_layers["tn"])
                if "tn" in self.transfer_learning.freeze_layers
                else 0
            )

            if num_frozen_layers_bn > self.branch_net.num_hidden_layers + 1:
                raise Exception(
                    f"ERROR: Number of branch layers to freeze exceeds the number of layers (hidden + out): {num_frozen_layers_bn}/{self.branch_net.num_hidden_layers + 1}"
                )
            if num_frozen_layers_bn == self.branch_net.num_hidden_layers + 1:
                print(
                    f"WARNING: all branch layers are frozen (hidden + out): {num_frozen_layers_bn}/{self.branch_net.num_hidden_layers + 1}"
                )

            if num_frozen_layers_tn > self.trunk_net.num_hidden_layers + 1:
                raise Exception(
                    f"ERROR: Number of trunk layers to freeze exceeds the number of layers (hidden + out): {num_frozen_layers_tn}/{self.trunk_net.num_hidden_layers + 1}"
                )
            if num_frozen_layers_tn == self.trunk_net.num_hidden_layers + 1:
                print(
                    f"WARNING: all trunk layers are frozen (hidden + out): {num_frozen_layers_tn}/{self.trunk_net.num_hidden_layers + 1}"
                )
        ...
        use_adaptive_weights = (
            settings["use_adaptive_weights"]
            if "use_adaptive_weights" in settings
            else False
        )
...
def setupNetworkArchitecture(net: dict, num_output_neurons: int):
    ...
    if architecture == "mlp":
        ...
        return MLPArchitecture(
            NetworkArchitectureType.MOD_MLP,
            activation,
            num_hidden_layers,
            num_hidden_neurons,
            num_output_neurons,
        ...
        return ResNetArchitecture(
            NetworkArchitectureType.RESNET,
            activation,
            num_hidden_layers,
            num_hidden_neurons,
            num_output_neurons,
