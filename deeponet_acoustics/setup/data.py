...
def setupDataResample(p, t, grid, dx_u, xminmax, yminmax, do_plot=False):
    ...
    if do_plot:
        ...
        ax = plt.axes(projection="3d")
        ax.plot_trisurf(
            grid1d[0 : (Npoints - 1), 0],
            grid1d[0 : (Npoints - 1), 1],
            s[0, t_indx_i * Npoints : ((t_indx_i + 1) * Npoints)],
        )
        plt.show(block=True)
...
def setupTransferLearningFromMat(
    transfer_learning: TransferLearning, flatten_data=True
):
    if transfer_learning.ceod is None:
        u_src_train = []
        u_src_val = []
    else:
        # r = 1541 # source square
        # r = 1200 # target right triangle
        u_src_train, u_src_val, *_ = rw.loadDataFromMat(
            transfer_learning.ceod.training_data_src_path,
...
def setupTrainingData(key, samples_srcs, p_srcs, xx, tt, P, N):
    ...
    u_train, y_train, s_train = vmap(setupOneTrainingData, (0, 0, 0, None, None, None))(
        keys, samples_srcs, p_srcs, np.array(xx), np.array(tt), P
    )
