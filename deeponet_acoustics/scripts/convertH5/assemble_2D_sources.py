...
def assemble_2d_source_files(data_dir, header_filepath_in, filepath_out):
    ...
    with h5py.File(filepath_out, "w") as fw:
        ...
        pressure_new = fw.create_dataset(
            "pressures", chunks=True, shape=shape_p, dtype=np.float32
        )
        upressure_new = fw.create_dataset(
            "upressures", chunks=True, shape=shape_up, dtype=np.float32
        )
        x0_srcs_new = fw.create_dataset(
            "x0_srcs", chunks=True, shape=shape_srcs, dtype=np.float32
        )
        ...
        with h5py.File(header_filepath_in, "r") as f:
            ...
            dx = mesh.attrs["dx"]
            dx_src = (
                mesh.attrs["dx_src"] if "dx_src" in mesh.attrs else []
            )  # only available in newer datasets
            c = mesh.attrs["c"]
