...
def create_metadata_json(src_folder: Path, pressure_attrs: dict, src_pos: np.ndarray | None) -> None:
    """
...
def split_2d_by_source_position(input_file: Path, output_path: Path):
    ...
    with h5py.File(input_file, 'r') as f_in:
        ...
        pressures = f_in['pressures'][:]  # Shape: (num_sources, time_steps, spatial_points)
        upressures = f_in['upressures'][:]  # Shape: (num_sources, uniform_grid_points)
        x0_srcs = f_in['x0_srcs'][:] if 'x0_srcs' in f_in else None  # Shape: (num_sources * spatial_dims)
        ...
        if num_sources != pressures.shape[0]:
            print(f"WARNING: There is a mismatch between number of x0_srcs ({num_sources}) and the actual sources used to simulate the pressure fields {pressures.shape[0]}. Source positions omitted.")
        ...
        for src_idx in range(num_sources):
            ...
            with h5py.File(output_file, 'w') as f_out:
                ...
                mesh_dataset.attrs['domain_minmax'] = mesh_attrs.get('domain_minmax', np.array([[0, 2], [0, 2]]))
                
                # Keep umesh as 2D
                umesh_dataset = f_out.create_dataset('umesh', data=umesh)
                umesh_dataset.attrs['umesh_shape'] = umesh_shape.astype(np.int64)  # Keep as int
                
                # Extract pressure data for this source (transpose to match 3D format)
                src_pressures = pressures[src_idx, :, :]  # From (src_pos, time_steps, spatial_points) to (time_steps, spatial_points)
                pressures_dataset = f_out.create_dataset('pressures', data=src_pressures)
                ...
                if x0_srcs is not None:
                    ...
                    src_pos_2d = x0_srcs[src_idx, :]  # Extract (spatial_dims,) position for this source
                    f_out.create_dataset('source_position', data=src_pos_2d.astype(np.float32))
                
                # Extract upressures for this source
                src_upressures = upressures[src_idx, :]  # Extract (uniform_grid_points,) for this source
                upressures_dataset = f_out.create_dataset('upressures', data=src_upressures)
...
def main():
    parser = argparse.ArgumentParser(description='Convert 2D H5 data format to 3D format')
    parser.add_argument('--input_file', required=True, type=Path, help='Input 2D H5 file path')
    parser.add_argument('--output_dir', required=True, type=Path, help='Output directory for 3D format files')
