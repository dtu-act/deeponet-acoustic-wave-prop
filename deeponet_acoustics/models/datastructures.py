...
class InputOutputDirs:
    ...
    def __init__(self, settings_dict, input_dir=None, output_dir=None):
        ...
        self.data_dir = settings_dict["input_dir"] if input_dir is None else input_dir
...
class TransferLearning:
    ...
    def __init__(self, settings_dict: dict, transfer_model_path: str):
        self.transfer_model_path = (
            settings_dict["transfer_learning"]["transfer_model_path"]
            if "transfer_model_path" in settings_dict["transfer_learning"]
            else transfer_model_path
        )
        self.freeze_layers = (
            settings_dict["transfer_learning"]["freeze_layers"]
            if "freeze_layers" in settings_dict["transfer_learning"]
            else dict()
        )
...
class EvaluationSettings:
    ...
    def __init__(self, settings: dict, num_srcs: int = -1):
        ...
        if key_src_pos in settings:
            if (
                not isinstance(settings["source_positions"], list)
                or not len(np.array(settings["source_positions"]).shape) == 2
            ):
                raise Exception(
                    "Source positions are explicitly set: Expected non-empty two-dimensional list."
                )
            ...
            for i_src in range(len(self.source_position_override)):
                self.source_position_override[i_src] = settings["source_positions"][
                    i_src
                ]
        else:
            if num_srcs <= 0:
                raise Exception(
                    "Number of source positions cannot be determined from the settings (source not set explicitly, instead loaded from disk), please provide it as input argument to the function"
                )
        ...
        if key_recv_groups in settings:
            if (
                not isinstance(settings[key_recv_groups], list)
                or len(settings[key_recv_groups]) == 0
                or not isinstance(settings[key_recv_groups][0], str)
            ):
                raise Exception(
                    "Expected non-empty list of string for key receiver_position_groups"
                )
            elif len(settings[key_recv_groups]) != self.num_srcs:
                raise Exception(
                    f"Number of receiver groups (receiver_position_groups) {len(settings[key_recv_groups])} differs from number of source {self.num_srcs}"
                )

            self.receiver_pos = self.parseReceiverGroups(
                settings[key_recv_groups], settings
            )
        elif key_recv_pos in settings:
            if (
                not isinstance(settings[key_recv_pos], list)
                or len(settings[key_recv_pos]) == 0
                or not isinstance(settings[key_recv_pos][0], list)
                or not len(np.array(settings[key_recv_pos]).shape) == 2
            ):
                raise Exception(
                    "Expected non-empty two-dimensional list of receiver coordinates ('receiver_positions_all_sources'). The same receivers are expected to be used for each source position and should NOT be repeated for each source (contrary to 'receiver_position_groups')"
                )
    ...
    def parseReceiverGroups(self, receiver_keys: list, receivers: dict) -> np.ndarray:
        ...
        for i_src in range(len(receiver_keys)):
            ...
            if recvs_key not in receivers:
                raise Exception(
                    f"The JSON key {recvs_key} for source index {i_src} was not found. Please add this key to the JSON file with corresponding receiver positions as value."
                )
