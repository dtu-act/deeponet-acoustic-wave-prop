...
def freezeLayersToKeys(freeze_layers: dict, freeze_b0=False):
    ...
    if "bn_transformer" in freeze_layers:
        freeze_set.add("transformerU_bn")
        freeze_set.add("transformerV_bn")
    if "tn_transformer" in freeze_layers:
        freeze_set.add("transformerU_tn")
        freeze_set.add("transformerV_tn")
...
def flattened_traversal(fn, network_keys):
    ...
    def mask(tree):
        ...
        for key in network_keys:
            if isinstance(tree[key], dict) or isinstance(
                tree[key], flax.core.FrozenDict
            ):
...
def sinusoidal_init(is_first=False):
    def init(
        key: ArrayLike, shape: ArrayLike, dtype: DTypeLikeInexact = dtype
    ) -> ArrayLike:
        d_in = shape[0]
        d_out = shape[1]
        ...
        W = random.uniform(
            key, (d_in, d_out), minval=minval, maxval=maxval, dtype=dtype
        )
...
def setupNetwork(
    net: NetworkArchitecture, tag: str, dim: int = -1
    ...
    if net.architecture == NetworkArchitectureType.MLP:
        layers = net.num_hidden_layers * [net.num_hidden_neurons] + [
        ...
        num_group_blocks = (
            net.num_group_blocks
        )  # : tuple = (3, 3, 3, 3) # todo: read from settings
        c_hidden = (
            net.cnn_hidden_layers
        )  # : tuple = (16, 32, 64, 128) # todo: read from settings
        layers = net.num_hidden_layers * [net.num_hidden_neurons] + [
            net.num_output_neurons
        ]
        kernel_size = (3,) * dim
...
class MLP(nn.Module):
    ...
    def __call__(self, inputs, output_layer_indx=-1):
        ...
        if output_layer_indx == -1:
            ...
            x = nn.Dense(
                features=self.layers[-1],
                kernel_init=self.kernel_init(),
                name=f"linear_{self.tag}_{len(self.layers) - 1}",
            )(x)
...
class ResNet(nn.Module):
    ...
    def __call__(self, x, output_layer_indx=-1, train=True):
        ...
        if output_layer_indx == -1:
            x = nn.Dense(
                features=self.layers_fnn[-1], kernel_init=self.kernel_sine_init()
            )(x)
...
class Cnn(nn.Module):
    ...
    def __call__(self, x, output_layer_indx=-1, train=True):
        ...
        x = nn.Dense(
            features=self.layers_fnn[-1], kernel_init=nn.initializers.xavier_uniform()
        )(x)
