...
def lossCEOD(
    params: dict,
    batch_target: list[float],
    branch_net: Callable,
    operator_net: Callable,
    ceod_indx: int = -1,
):
    """
    ...
    u_tar, y_tar, u_src = (
        inputs_tar  # make sure to modify the data loader to return u_src
    )

    assert len(u_src) > 0

    # RESIDUAL LOSS - forward pass
    branch_latent_batch = vmap(branch_net, (None, 0))(
        params, u_tar
    )  # vmaps along branch batch dimension
    s_pred_tar = s_pred_fn(
        params, branch_latent_batch, y_tar
    )  # calculate predictions on coordinate locations
    ...
    s_pred_src = s_pred_fn(
        params, branch_latent_batch, y_tar
    )  # calculate predictions on coordinate locations

    # get output from layer 'ceod_indx' for both source and target data
    branch_discrepancy_layer_tar = vmap(branch_net, (None, 0, None))(
        params, u_tar, ceod_indx
    )
    branch_discrepancy_layer_src = vmap(branch_net, (None, 0, None))(
        params, u_src, ceod_indx
    )

    loss_ceod = calcCEOD(
        branch_discrepancy_layer_tar,
        outputs_tar,
        branch_discrepancy_layer_src,
        s_pred_src,
    )
