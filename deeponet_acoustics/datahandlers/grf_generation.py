...
def generate_grf_function_mask_1d(
    xmin, xmax, sigma_0, l_0, num_of_elements_x, num_of_samples, sigma0_window=0.2
):
    ...
    for mode_index, corr_length in enumerate(l_0):
        ...
        covariance_matrix += (
            1
            / (sigma_0[mode_index] * np.sqrt(2 * np.pi))
            * np.exp(-0.5 / (corr_length**2) * distances_squared)
        )
...
def generate_grf_function_2d(
    sigma_0, l_0, num_of_elements_x, num_of_elements_y, num_of_samples
):
    if len(sigma_0) != len(l_0):
        raise ValueError("Size of sigma_0 and l_0 mismatch")
    ...
    covariance_matrix = np.zeros((num_of_nodes, num_of_nodes))
    for mode_index, corr_length in enumerate(l_0):
        covariance_matrix += (sigma_0[mode_index] ** 2) * np.exp(
            -0.5 / (corr_length**2) * distances_squared
        )
    mu = np.zeros_like(x)
    samples = np.random.multivariate_normal(mu, covariance_matrix, num_of_samples)
...
def generate_grf_function_mask_2d(
    xminmax, yminmax, sigma_0, l_0, x1d, y1d, num_of_samples, sigma0_window
):
    if len(sigma_0) != len(l_0):
        raise ValueError("Size of sigma_0 and l_0 mismatch")
    if len(x1d) != len(y1d):
    ...
    gauss_fn = lambda x, y, x0, y0: np.exp(
        -(((x - x0) ** 2 + (y - y0) ** 2) / sigma0_window**2)
    )
...
def generate_grf_function_mask_3d(
    xminmax,
    yminmax,
    zminmax,
    sigma_0,
    ...
    covariance_matrix += (
        1 / (sigma_0 * np.sqrt(2 * np.pi)) * np.exp(-0.5 / (l_0**2) * distances_squared)
    )
    mu = np.zeros_like(x1d)
    samples = np.random.multivariate_normal(mu, covariance_matrix, num_of_samples)
    ...
    gauss_fn = lambda x, y, z, x0, y0, z0: np.exp(
        -(((x - x0) ** 2 + (y - y0) ** 2 + (z - z0) ** 2) / sigma0_window**2)
    )
