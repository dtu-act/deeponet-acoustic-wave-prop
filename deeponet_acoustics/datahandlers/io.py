...
class XdmfReader():
    ...
    def __init__(self, filename, tmax=float('inf')):
        ...
        self.tsteps = list(map(lambda x: float(x.attrib['Value']), root.findall(".//Time")))
...
def pathsToFileType(path, filetype, exclude=''):
    paths_to_file = []
    parent_dir_list = [i for i in os.listdir(path) if os.path.isdir(os.path.join(path,i))]
    ...
    for parent_dir_name in parent_dir_list:
        ...
        for filename in os.listdir(data_dir):
            if filename.endswith(filetype) and (exclude == '' or filename.find(exclude) == -1):
...
def writeXdmf(grid, t_range, p, filename, cells=[]):
    if len(cells) == 0:
        vertex = np.array([[i,] for i in range(len(grid))])    
        cells = [
            ("vertex", vertex)
        ]
